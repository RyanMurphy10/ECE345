...
SoftwareSerial serialLCD = SoftwareSerial(-1, 7, 1); //Declare the SoftwareSerial object

const char clearScreen[5] = {0xFE, 0x01, 0xFE, 0x80, 0x00}; //Define the clear screen command for the LCD

void setup() //Setup Code...Initialize Hardware
{
  pinMode(7, OUTPUT);
  digitalWrite(7, LOW);
  serialLCD.begin(9600);
  serialLCD.print(clearScreen);
  loadCell.begin(3,2);
...
void loop()
{
  ...
  switch(machineState)
  {
    case calibration:
      if ((serialInput == 'M') || (serialInput == 'm'))
      {
        ...
        machineState = measurement; //Sets to Measurement Mode
      }
    ...
    case measurement:
      if ((serialInput == 'C') || (serialInput == 'c'))
      {
        ...
        machineState = calibration; //Sets to Calibration Mode
      }
      break;
    default:
      break;
  ...
  if (loopDelay > 40000)
  {
    loopDelay = 0; //Sets loopDelay to 0
    digitalWrite(13, !digitalRead(13));
    //Update LCD with load cell reading
    serialLCD.print(clearScreen); //Clear LCD screen
    serialLCD.print(loadCell.get_units(), 3); //Display weight with 3 decimal places
  }
}
